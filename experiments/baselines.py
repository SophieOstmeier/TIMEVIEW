...
class BaseBenchmark(ABC):
    ...
    def run(
        ...
        **kwargs,
        ...
        if n_tune > 0:
            ...
            best_hyperparameters = self.tune(
                n_trials=n_tune, seed=tuning_seed, benchmarks_dir=benchmarks_dir
            )
        else:
    ...
    @abstractmethod
    def prepare_data(
        self, dataset: BaseDataset, train_indices, val_indices, test_indices
    ):
...
class PySRBenchmark(BaseBenchmark):
    ...
    def prepare_data(
        ...
        joblib.dump(
            column_transformer,
            os.path.join(self.benchmarks_dir, self.name, "column_transformer.joblib"),
        )
    ...
    def train(self, model, tuning=False):
        ...
        for i in range(n_equations):
            val_losses.append(
                mean_squared_error(self.y_val, model.predict(self.X_val, index=i))
            )
...
class SINDyBenchmark(BaseBenchmark):
    ...
    def get_model_for_tuning(self, trial, seed):
        ...
        optimizer_threshold = trial.suggest_float(
            "optimizer_threshold", 1e-3, 1e-1, log=True
        )
        optimizer = ps.STLSQ(threshold=optimizer_threshold)

        differentiation_kind = trial.suggest_categorical(
            "differentiation_kind", ["finite_difference", "spline", "trend_filtered"]
        )
        if differentiation_kind == "finite_difference":
            ...
            differentiation_method = ps.SINDyDerivative(
                kind="trend_filtered", order=order, alpha=alpha
            )
        ...
        lib_poly = ps.PolynomialLibrary(
            degree=2, include_interaction=True, include_bias=True
        )

        model = ps.SINDy(
            optimizer=optimizer,
            differentiation_method=differentiation_method,
            feature_library=lib_poly,
        )
    ...
    def train(self, model, tuning=False):
        ...
        if not tuning:
            if self.fitted:
                return (
                    self.results
                )  # We can do it because the algorithm is deterministic
        ...
        if tuning:
            ...
            for X, t, y in zip(self.X_val, self.ts_val, self.ys_val):
                try:
                    ...
                    # print("Simulating")
                    u = get_control_function(X)
                    y_pred = clip_to_finite(
                        model.simulate(y[[0]], t=t, u=u, integrator="odeint")
                    )
                except Exception as e:
                    print(e)
...
class GAMBenchmark(BaseBenchmark):
    ...
    def get_model_for_tuning(self, trial, seed):
        ...
        return ExplainableBoostingRegressor(
            **parameters, interactions=0, random_state=seed
        )
...
class CatBoostBenchmark(BaseBenchmark):
    ...
    def get_model_for_tuning(self, trial, seed):
        ...
        parameters = {
            ...
            "early_stopping_rounds": trial.suggest_int(
                "early_stopping_rounds", 10, 50, step=10
            ),
        }
    ...
    def prepare_data(
        ...
        self.categorical_indices = [
            i
            for i, name in enumerate(dataset.get_feature_names())
            if dataset.get_feature_type(name) == "categorical"
        ]

        self.X_train = pd.DataFrame(
            self.X_train, columns=dataset.get_feature_names() + ["t"]
        )
        for i in self.categorical_indices:
            self.X_train.iloc[:, i] = (
                self.X_train.iloc[:, i].astype(int).astype("category")
            )

        self.X_val = pd.DataFrame(
            self.X_val, columns=dataset.get_feature_names() + ["t"]
        )
        for i in self.categorical_indices:
...
class LGBMBenchmark(BaseBenchmark):
    ...
    def get_model_for_tuning(self, trial, seed):
        ...
        parameters = {
            ...
            "reg_alpha": (
                trial.suggest_float("reg_alpha", 1e-7, 10.0, log=True)
                if if_alpha
                else 0.0
            ),
            "reg_lambda": (
                trial.suggest_float("reg_lambda", 1e-7, 10.0, log=True)
                if if_lambda
                else 0.0
            ),
            "n_jobs": -1,
    ...
    def train(self, model, tuning=False):
        ...
        if tuning:
            log_dir = os.path.join(
                ...
                "tuning",
                "logs",
                f'seed_{model.get_params()["random_state"]}',
            )
        else:
            log_dir = os.path.join(
                self.benchmarks_dir,
    ...
    def prepare_data(
        ...
        if self.config.n_basis_tunable:
            ...
            self.train_dataset = TTSDataset(
                self.config, (self.X_train, self.ts_train, self.ys_train)
            )
            self.val_dataset = TTSDataset(
                self.config, (self.X_val, self.ts_val, self.ys_val)
            )
            self.test_dataset = TTSDataset(
                self.config, (self.X_test, self.ts_test, self.ys_test)
            )
    ...
    def train(self, model, tuning=False):
        ...
        if self.config.n_basis_tunable:
            ...
            train_dataset = TTSDataset(
                model.config, (self.X_train, self.ts_train, self.ys_train)
            )
            val_dataset = TTSDataset(
                model.config, (self.X_val, self.ts_val, self.ys_val)
            )
            test_dataset = TTSDataset(
                model.config, (self.X_test, self.ts_test, self.ys_test)
            )
        else:
        ...
        best_val_checkpoint = pl.callbacks.ModelCheckpoint(
            monitor="val_loss", mode="min", save_top_k=1, filename="best_val"
        )
        # added early stopping callback, if validation loss does not improve over 10 epochs -> terminate training.
        early_stop_callback = pl.callbacks.EarlyStopping(
            monitor="val_loss", min_delta=0, patience=10, verbose=False, mode="min"
        )
        ...
        train_dataloader = create_dataloader(
            model.config, train_dataset, None, shuffle=True
        )
        val_dataloader = create_dataloader(
            model.config, val_dataset, None, shuffle=False
        )
        test_dataloader = create_dataloader(
            model.config, test_dataset, None, shuffle=False
        )
...
class RNNBenchmark(BaseBenchmark):
    ...
    def prepare_data(
        ...
        if not all([self.config.max_len == y_length for y_length in y_lengths]):
            raise ValueError(
                "All y values must have the same length if you want to use RNN. Same as max_len parameter of RNN."
            )
        ...
        self.train_dataset = torch.utils.data.TensorDataset(
            ...
            torch.stack(
                [torch.tensor(y, dtype=torch.float32) for y in self.ys_train], dim=0
            ),
        )
        self.val_dataset = torch.utils.data.TensorDataset(
            torch.tensor(self.X_val, dtype=torch.float32),
            torch.stack(
                [torch.tensor(y, dtype=torch.float32) for y in self.ys_val], dim=0
            ),
        )
        self.test_dataset = torch.utils.data.TensorDataset(
            torch.tensor(self.X_test, dtype=torch.float32),
            torch.stack(
                [torch.tensor(y, dtype=torch.float32) for y in self.ys_test], dim=0
            ),
        )
...
class DeltaTRNNBenchmark(BaseBenchmark):
    ...
    def prepare_data(
        ...
        self.train_dataset = prepare_tensor_dataset(
            self.X_train, self.ts_train, self.ys_train
        )
        self.val_dataset = prepare_tensor_dataset(self.X_val, self.ts_val, self.ys_val)
        self.test_dataset = prepare_tensor_dataset(
            self.X_test, self.ts_test, self.ys_test
        )
