...
def save_dataset(
    ...
    if os.path.exists(path):
        raise ValueError(
            f"A dataset description file with this name already exists at {path}."
        )
...
def load_dataset_description(
    ...
    if not os.path.exists(path):
        raise ValueError(
            f"A dataset description file with this name does not exist at {path}."
        )
...
def load_dataset(
    dataset_name, dataset_description_path="dataset_descriptions", data_folder=None
):
    dataset_description = load_dataset_description(
        dataset_name, dataset_description_path=dataset_description_path
    )
    dataset_builder = dataset_description["dataset_builder"]
    dataset_dictionary = dataset_description["dataset_dictionary"]
    if data_folder is not None:
        print(
            "Overriding data folder with",
            "/dataNAS/people/sostm/TIMEVIEW/experiments/data",
        )
...
class BetaDataset(BaseDataset):
    ...
    def __init__(self, n_samples=100, n_timesteps=20):
        ...
        self.ys = [
            np.array(
                [beta.pdf(t, alpha, betap) for t in np.linspace(0, 1, n_timesteps)]
            )
    ...
    def get_feature_ranges(self):
        return {"alpha": (1.0, 4.0), "beta": (1.0, 4.0)}
...
class FLChainDataset(BaseDataset):
    ...
    def __init__(self, subset="all", data_folder=None):
        super().__init__(subset=subset)
        if data_folder is None:
            df = pd.read_csv(os.path.join("data", "flchain", "flchain.csv"))
        else:
            df = pd.read_csv(os.path.join(data_folder, "flchain", "flchain.csv"))
        df = df[["id"] + self.get_feature_names() + ["t", "y"]]
...
class StressStrainDataset(BaseDataset):
    ...
    def __init__(
        ...
        super().__init__(
            lot=lot,
            include_lot_as_feature=include_lot_as_feature,
        ...
        for filename in filenames:
            ...
            )  # drop last row because it's an outlier
            if downsample:
            ...
            if more_samples > 0:
                for ind in range(more_samples):
                    df.loc[ind::more_samples, "id"] = (
                        df.loc[ind::more_samples, "id"] + "_" + str(ind)
                    )
            dfs.append(df)
        ...
        df.drop(
            index=df[df["t"] > max_strain].index, inplace=True
        )  # drop rows where t is > max_strain
        df["y"] = df["y"] / 300  # scale
...
class TacrolimusDataset(BaseDataset):
    ...
    def __init__(self, granularity, normalize=False, max_t=25, data_folder="data"):
        ...
        if granularity == "visit":
            ...
            df = pd.read_csv(
                os.path.join(data_folder, "tacrolimus", "tac_pccp_mr4_250423.csv")
            )
            dosage_rows = df[df["DOSE"] != 0]
            assert dosage_rows["visit_id"].is_unique
            df.drop(
                columns=["DOSE", "EVID", "II", "AGE"], inplace=True
            )  # we drop age because many missing values. the other columns are not needed
            df.drop(index=dosage_rows.index, inplace=True)  # drop dosage rows
            # Merge df with dosage rows on visit_id
            df = df.merge(
                dosage_rows[["visit_id", "DOSE"]], on="visit_id", how="left"
            )  # add dosage as a feature
            df.loc[
                df["TIME"] >= 168, "TIME"
            ] -= 168  # subtract 168 from time to get time since last dosage
            missing_24h = df[(df["TIME"] == 0) & (df["DV"] == 0)].index
            df.drop(
                index=missing_24h, inplace=True
            )  # drop rows where DV is 0 and time is 0 - they correspond to missing 24h measurements

            dv_0 = df[df["TIME"] == 0][["visit_id", "DV"]]
            assert dv_0["visit_id"].is_unique
            df = df.merge(
                dv_0, on="visit_id", how="left", suffixes=("", "_0")
            )  # add DV_0 as a feature

            more_than_t = df[df["TIME"] > max_t].index
            df.drop(
                index=more_than_t, inplace=True
            )  # drop rows where time is greater than max_t
            ...
            raise NotImplementedError(
                "Only visit granularity is implemented for this dataset."
            )
...
class WindDataset(BaseDataset):
    ...
    def __init__(self, company, granularity="daily", rolling=False):
        ...
        def load_company(name):
            ...
            df = df.melt(
                id_vars=["id", "day_number", "month"], var_name="time", value_name="y"
            )
...
class TumorDataset(BaseDataset):
    ...
    def __init__(self, **args):
        ...
        first_measurements = pd.merge(
            first_time, df[["name", "date", "size"]], on=["name", "date"]
        )
...
class SyntheticTumorDataset(BaseDataset):
    ...
    def __init__(self, **args):
        ...
        if self.args.equation == "wilkerson":
            self.X = pd.DataFrame(
                X, columns=["age", "weight", "initial_tumor_volume", "dosage"]
            )
    ...
    def _tumor_volume(t, age, weight, initial_tumor_volume, start_time, dosage):
        ...
        def C(t):
            return np.where(
                t < start_time, 0.0, dosage * np.exp(-GAMMA_0 * (t - start_time))
            )
    ...
    def synthetic_tumor_data(
        ...
        # Sample age
        age = gen.uniform(
            TUMOR_DATA_FEATURE_RANGES["age"][0],
            TUMOR_DATA_FEATURE_RANGES["age"][1],
            size=n_samples,
        )
        # Sample weight
        weight = gen.uniform(
            TUMOR_DATA_FEATURE_RANGES["weight"][0],
            TUMOR_DATA_FEATURE_RANGES["weight"][1],
            size=n_samples,
        )
        # Sample initial tumor volume
        tumor_volume = gen.uniform(
            TUMOR_DATA_FEATURE_RANGES["initial_tumor_volume"][0],
            TUMOR_DATA_FEATURE_RANGES["initial_tumor_volume"][1],
            size=n_samples,
        )
        # Sample start time of chemotherapy
        start_time = gen.uniform(
            TUMOR_DATA_FEATURE_RANGES["start_time"][0],
            TUMOR_DATA_FEATURE_RANGES["start_time"][1],
            size=n_samples,
        )
        # Sample chemotherapy dosage
        dosage = gen.uniform(
            TUMOR_DATA_FEATURE_RANGES["dosage"][0],
            TUMOR_DATA_FEATURE_RANGES["dosage"][1],
            size=n_samples,
        )
        ...
        for i in range(n_samples):
            ...
            if equation == "wilkerson":
                ys.append(
                    SyntheticTumorDataset._tumor_volume_2(
                        ts[i], age, weight, tumor_volume, dosage
                    )
                )
            elif equation == "geng":
                ys.append(
                    SyntheticTumorDataset._tumor_volume(
                        ts[i], age, weight, tumor_volume, start_time, dosage
                    )
                )
