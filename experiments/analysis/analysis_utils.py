...
def find_results(
    ...
    with open(os.path.join(results_dir, summary_filename), "r") as summary_file:
        ...
        # print(f"Loaded {len(summary)} results from {summary_filename}")

    for result in summary:
        # print(f"Found: {result['dataset_name']}")
...
def load_result_from_timestamp(
    timestamp, results_dir="../benchmarks", summary_filename="summary.json"
):
    with open(os.path.join(results_dir, summary_filename), "r") as summary_file:
        summary = json.load(summary_file)
...
def check_if_dataset_description_exists(
    dataset_name, dataset_descriptions_path="../dataset_descriptions"
):
    ...
    if not os.path.exists(dataset_path):
        raise ValueError(
            "Dataset description for {} does not exist".format(dataset_name)
        )
    return
...
def create_shap_plot(
    ...
    column_transformer = load_column_transformer(
        timestamp, model_name, benchmarks_dir=results_dir
    )
    y_normalizer = YNormalizer.load_from_benchmark(
        timestamp, model_name, benchmark_dir=results_dir
    )
    ...
    plt.title(
        f"SHAP values for each feature over time ({dataset_title} dataset)", fontsize=14
    )
    return fig
