...
class TTS(torch.nn.Module):
    ...
    def forward(self, X, Phis):
        ...
        if self.config.dataloader_type == "iterative":
            if is_dynamic_bias_enabled(self.config):
                return [
                    torch.matmul(Phi, h[d, :]) + self.bias[d]
                    for d, Phi in enumerate(Phis)
                ]
            else:
                return [
                    torch.matmul(Phi, h[d, :]) + self.bias for d, Phi in enumerate(Phis)
                ]
        elif self.config.dataloader_type == "tensor":
            if is_dynamic_bias_enabled(self.config):
                return torch.matmul(Phis, torch.unsqueeze(h, -1)).squeeze(
                    -1
                ) + torch.unsqueeze(self.bias, -1)
            else:
                return (
                    torch.matmul(Phis, torch.unsqueeze(h, -1)).squeeze(-1) + self.bias
                )
    ...
    def forecast_trajectories(self, X, t):
        ...
        with torch.no_grad():
            ...
            if is_dynamic_bias_enabled(self.config):
                ...
                h = h[:, :-1]
            return (
                (torch.matmul(h, Phi.T) + self.bias).cpu().numpy()
            )  # shape (D,N), broadcasting will take care of the bias
