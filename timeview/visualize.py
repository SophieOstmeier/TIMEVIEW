...
def simple_interactive_plot(
    f, time_horizon, trajectory_range, feature_ranges, n_points=1000, figsize=(8, 3)
):
    """
...
def draw_rectangles(transitions, colors):
    ...
    for i in range(len(widths)):
        rect_width = widths[i]
        rect = plt.Rectangle(
            (x, 0), rect_width, total_height, color=colors[i], alpha=1.0, linewidth=0.0
        )
        ax.add_patch(rect)
        x += rect_width
...
def get_meta_template(litmodel, features, index, query_points):
    ...
    templates = [
        bspline.get_template_from_coeffs(coeffs[i, :])[0]
...
class MetaTemplateContext:
    ...
    def update(self, raw_features):
        ...
        self.current_transformed_features = (
            self.column_transformer.transform(raw_features)
            if self.column_transformer
            else raw_features.values
        )  # This is 2D
        self.combined_templates = []
        self.transitions = []
        for feature_name in self.feature_names:
            combined_templates, transitions = self._get_meta_template(
                raw_features, feature_name
            )
            self.combined_templates.append(combined_templates)
            self.transitions.append(transitions)

        coeffs = self.litmodel.model.predict_latent_variables(
            self.current_transformed_features
        )
        template, transition_points = self.bspline.get_template_from_coeffs(
            coeffs[0, :]
        )
        ...
        for x_axis in self.feature_names:
            query_points, trans_x_all, trans_y_all = self._get_transition_point_curves(
                raw_features, x_axis
            )
            self.transition_points_trajectories[x_axis] = {
                "query_points": query_points,
                "transition_points": [
                    {"t": trans_x_all[index], "y": trans_y_all[index]}
    ...
    def _get_query_points(self, feature_name):
        if self.feature_types[feature_name] == "continuous":
            return np.linspace(
                self.feature_ranges[feature_name][0],
                self.feature_ranges[feature_name][1],
                self.n_points,
            )
        elif (
            self.feature_types[feature_name] == "categorical"
            or self.feature_types[feature_name] == "binary"
        ):
    ...
    def _get_meta_template(self, raw_features, feature_name):
        ...
        new_features = pd.concat(
            [raw_features] * len(query_points), axis=0, ignore_index=True
        )
        ...
        templates = [
            self.bspline.get_template_from_coeffs(coeffs[i, :])[0]
            for i in range(len(query_points))
        ]
    ...
    def _get_transition_point_curves(self, raw_features, x_axis):
        ...
        combined_templates, transitions = self.get_meta_templates_and_transitions(
            x_axis_index
        )
        curr_x_axis_value = raw_features.loc[0, x_axis]

        if (
            self.feature_types[x_axis] == "categorical"
            or self.feature_types[x_axis] == "binary"
        ):
            numerical_transitions = [
                self.feature_ranges[x_axis].index(transition)
                for transition in transitions
            ] + [len(self.feature_ranges[x_axis])]
            curr_x_axis_value_numerical = self.feature_ranges[x_axis].index(
                curr_x_axis_value
            )
            first_transition_point_on_the_right = bisect_right(
                numerical_transitions, curr_x_axis_value_numerical
            )
            ...
            query_points = [
                self.feature_ranges[x_axis][i] for i in range(prev_point, next_point)
            ]

        elif self.feature_types[x_axis] == "continuous":
            first_transition_point_on_the_right = bisect_left(
                transitions, curr_x_axis_value
            )
            ...
            if first_transition_point_on_the_right == 0:
                ...
                x_axis_range = (
                    transitions[first_transition_point_on_the_right - 1],
                    transitions[first_transition_point_on_the_right],
                )
            eps = (
                self.feature_ranges[x_axis][1] - self.feature_ranges[x_axis][0]
            ) / self.n_points
            query_points = np.linspace(
                x_axis_range[0] + eps, x_axis_range[1] - eps, self.n_points
            )
        ...
        trans_x_coords_combined = [
            self.bspline.get_template_from_coeffs(coeffs[i, :])[1]
            for i in range(len(query_points))
        ]
        ...
        trans_x_coords_all = [
            np.array(
                [trans_x_coords_combined[q][index] for q in range(len(query_points))]
            )
        ...
        trans_y_coords_all = [
            np.diagonal(
                self.litmodel.model.forecast_trajectories(
                    features_tiled, trans_x_coords_all[index]
                )
            )
            for index in range(n_transitions)
        ]
    ...
    def get_transition_point_curve(self, x_axis, y_axis, index):
        return (
            self.transition_points_trajectories[x_axis]["query_points"],
            self.transition_points_trajectories[x_axis]["transition_points"][index][
                y_axis
            ],
        )
    ...
    def get_current_trajectory(self):
        ...
        y = self.litmodel.model.forecast_trajectory(
            self.current_transformed_features[0, :], t
        )
        return t, y

    def get_current_transition_points(self):
        transition_points_x = np.array(self.current_transition_points)
        transition_points_y = self.litmodel.model.forecast_trajectory(
            self.current_transformed_features[0, :], transition_points_x
        )
...
def advanced_tts_plot(
    litmodel, dataset, trajectory_range, n_points=100, figsize=(8, 3)
):
    ...
    def update(**x):
        ...
        for i in range(len(feature_names)):
            combined_templates, transitions = get_meta_template(
                litmodel,
                features,
                i,
                np.linspace(
                    feature_ranges[feature_names[i]][0],
                    feature_ranges[feature_names[i]][1],
                    100,
                ),
            )
            colors_to_use = []
        ...
        transition_points_y = litmodel.model.forecast_trajectory(
            features, np.array(transition_points)
        )
        with main_plot:
            main_plot.clear_output(wait=True)
            line.set_data(t, y)
...
def expert_tts_plot(
    ...
    if default_values is None:
        default_values = {
            feature_name: feature_ranges[feature_name][0]
    ...
    for k, v in feature_ranges.items():
        if feature_types[k] == "categorical" or feature_types[k] == "binary":
            ...
            sliders[k] = FloatSlider(
                min=v[0], max=v[1], step=step, value=default_values[k]
            )
    ...
    random_color_iter = iter(random_color())
    color_map = {}
    meta_template_context = MetaTemplateContext(
        litmodel,
        feature_names,
        feature_ranges,
        feature_types,
    ...
    def update(**x):
        ...
        for i in range(len(feature_names)):
            combined_templates, transitions = (
                meta_template_context.get_meta_templates_and_transitions(i)
            )
            if (
                feature_types[feature_names[i]] == "categorical"
                or feature_types[feature_names[i]] == "binary"
            ):
                transitions_numerical = [
                    feature_ranges[feature_names[i]].index(f) for f in transitions
                ]
                transitions = transitions_numerical + [
                    len(feature_ranges[feature_names[i]])
                ]
        ...
        if new_template:
            ...
            transition_point_dropdown.options = ["?"] + list(
                range(1, num_transition_points + 1)
            )
            transition_point_dropdown.value = "?"
        else:
            update_secondary(
                transition_point=transition_point_dropdown.value,
        ...
        transition_points_x, transition_points_y = (
            meta_template_context.get_current_transition_points()
        )
        ...
        with main_plot:
            ...
            scat.set_offsets(
                np.c_[transition_points_x, transform_y(transition_points_y)]
            )
            display(main_fig)
    ...
    def update_secondary(**x):
        ...
        with secondary_plot:
            ...
            if transition_point is not None:
                x_axis = x["x_axis"]
                y_axis = x["y_axis"]
                x_values, y_values = meta_template_context.get_transition_point_curve(
                    x_axis, y_axis, transition_point
                )
                ...
                if feature_types[x_axis] == "continuous":
                    ...
                    feature_types[x_axis] == "categorical"
                    or feature_types[x_axis] == "binary"
                ):
                    ...
                    for x_value in x_values:
                        if x_value not in colors_for_bar_plots[x_axis]:
                            colors_for_bar_plots[x_axis][x_value] = next(
                                random_color_iter
                            )
    ...
    interactive_output(
        update_secondary,
        {
            "transition_point": transition_point_dropdown,
    ...
    def update(**x):
        ...
        with heatmap:
            ...
            grid_ax.contourf(
                ...
                grid_y,
                grid_labels,
                levels=len(unique_labels) - 1,
                cmap=colormap,
                alpha=0.5,
            )
            grid_ax.scatter(
                x=x[feature_names[0]], y=x[feature_names[1]], c="black", s=20, zorder=2
            )
            plt.axis("off")
            # Show the colorbar
